    ...
    <main className="min-h-screen bg-transparent overflow-hidden relative font-sans">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-20">
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl p-8 md:p-12 shadow-2xl shadow-indigo-500/10">
          <h1 className="text-4xl font-extrabold text-slate-900 mb-8 tracking-tight">{t('legal.terms')}</h1>
          
          <div className="prose prose-slate max-w-none space-y-6 text-slate-600 leading-relaxed">
            <p className="text-sm font-medium text-slate-400">{t('legal.last_updated')}</p>
            
            <section className="space-y-3">
              <h2 className="text-xl font-bold text-slate-900 pt-4">{t('legal.t1_title')}</h2>
              <p>{t('legal.t1_content')}</p>
            </section>
            
            <section className="space-y-3">
              <h2 className="text-xl font-bold text-slate-900 pt-4">{t('legal.t2_title')}</h2>
              <p>{t('legal.t2_content')}</p>
            </section>
            
            <section className="space-y-3">
              <h2 className="text-xl font-bold text-slate-900 pt-4">{t('legal.t3_title')}</h2>
              <p>{t('legal.t3_content')}</p>
            </section>
            
            <section className="space-y-3">
              <h2 className="text-xl font-bold text-slate-900 pt-4">{t('legal.t4_title')}</h2>
              <p>{t('legal.t4_content')}</p>
            </section>
